...
The admission bar gates exit as well as entry: an entry that no longer clears it is removed, not kept for safety. An entry dies when its principle has become the default — no competent contributor would get it wrong anymore — or when the decision it guards can no longer arise.

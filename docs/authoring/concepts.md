...
The admission bar gates exit as well as entry: an entry that no longer clears it is removed, not kept for safety. An entry dies when no surface still uses the term, or when its contract is no longer enforced and its ambiguity is gone. A concept that stops being separable from a neighbor merges into it and survives only as an alias line.

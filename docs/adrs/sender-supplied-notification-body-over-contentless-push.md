...
- **A Rome-side preview preference over notification content.** Rejected because the device already hands the guardian a per-device control over exactly this exposure (iOS Show Previews, with Never as an option; Android's equivalent lock-screen controls). A Rome toggle over the same exposure is a second switch on one policy, and the guardian then reasons about the pair instead of the one setting the device already honors.
...
- Rome adds no parallel preview preference. Lock-screen exposure control stays with the device's own lock-screen preview setting (iOS Show Previews; Android's equivalent).

...
- When reviewing CI, inspect `shadcn lint (advisory)` even if CI is green. Its summary and diagnostic log groups contain non-blocking findings. Treat an incomplete scan as unavailable, never clean. Do not rewrite valid tokens or unrelated UI to silence this check.

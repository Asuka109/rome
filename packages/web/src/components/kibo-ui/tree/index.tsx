...
export const TreeProvider = ({
  ...
  const isExpandControlled = controlledExpandedIds !== undefined && onExpandedChange !== undefined;
  const expandedIds = isExpandControlled ? new Set(controlledExpandedIds) : internalExpandedIds;
  ...
  const toggleExpanded = useCallback(
    (nodeId: string) => {
      if (isExpandControlled) {
        const newSet = new Set(controlledExpandedIds);
    ...
    [isExpandControlled, controlledExpandedIds, onExpandedChange],
  );
...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        "group relative mx-1 flex items-center rounded-8 px-3 py-2 transition-all duration-200",
        "hover:bg-accent/60",
...
export const TreeExpander = ({
  ...
  return (
    <motion.div
      ...
      className={cn("mr-1 flex h-4 w-4 items-center justify-center", className)}
      onClick={(e: React.MouseEvent<HTMLDivElement>) => {
        e.stopPropagation();
        toggleExpanded(nodeId);
        onClick?.(e);
...
export const TreeLabel = ({ className, ...props }: TreeLabelProps) => (
  <span className={cn("font flex-1 truncate text-ui", className)} {...props} />
);

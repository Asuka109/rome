...
describe("Native logout", () => {
  it("clears Native sessions, WebView cookies, and selection after revocation", async () => {
    ...
    const clearCookie = vi.fn(async () => undefined);
    const clearSelection = vi.fn(async () => undefined);
    await logoutNativeSession({
      cloudApi: { revoke: vi.fn(async () => true) } as unknown as CloudApi,
      vault,
      cookieStore: { install: vi.fn(), clear: clearCookie },
      clearSelection,
    });
  ...
  it("quarantines and later retries a credential when Cloud is offline", async () => {
    ...
    await logoutNativeSession({
      ...
      vault,
      cookieStore: { install: vi.fn(), clear: vi.fn() },

...
- Upgrading an app is `pnpm up @rome-os/ui` from `<app-root>` (`--latest` to cross a `0.x` minor), not a re-scaffold. Crossing a minor only works if `styles.css` imports `@rome-os/ui/styles.css` itself, as the scaffold does — see [`REFERENCE.md` → Styling](./REFERENCE.md#styling) for why the SDK's import alone is not enough.
